...
public class Bomber extends Bomberman {
    ...
    public void update() {
        chooseSprite();
        checkEnemy();
        if (bomber.getLife() == 0 || bomber.getState() == 1 ) {
            entities.clear();
            stillObjects.clear();
    ...
    public void checkEnemy(){
        for (Entity entity : entities){
            if(entity instanceof Enemy){
                if(position[this.x/Sprite.SCALED_SIZE][this.y/Sprite.SCALED_SIZE] !=2
                        &&position[this.x/Sprite.SCALED_SIZE][this.y/Sprite.SCALED_SIZE]!=3) {
